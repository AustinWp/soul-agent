"""MCP (Model Context Protocol) server for mem-agent.

Exposes memory tools and resources over the MCP protocol so that
LLM clients (Claude Desktop, etc.) can interact with the mem-agent
...
def _call_daemon(method: str, path: str, **kwargs: Any) -> dict | str:
    """Make an HTTP request to the mem-agent daemon.
    ...
    try:
        ...
        return "Error: mem-agent daemon is not running. Start it with 'mem service start'."
