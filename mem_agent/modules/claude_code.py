...
DAEMON_PORT = 8330
HOOK_MARKER = "mem-agent-claude-code-hook"
...
def build_hook_config() -> dict:
    ...
    pipes post-tool-use summaries to the mem-agent daemon.
    """
    return {
        "hooks": {
            "postToolUse": [
...
def install_hook() -> None:
    """Merge the mem-agent hook config into Claude Code settings.json.
...
def uninstall_hook() -> None:
    """Remove the mem-agent hook from Claude Code settings.json."""
