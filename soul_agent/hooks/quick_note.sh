#!/bin/bash
# soul-agent quick note — macOS shortcut-triggered note input
...
if [ "$http_code" = "200" ]; then
    osascript -e 'display notification "Note saved to memory." with title "soul-agent"' 2>/dev/null
else
    osascript -e 'display notification "Failed to save note (is daemon running?)." with title "soul-agent"' 2>/dev/null
fi
