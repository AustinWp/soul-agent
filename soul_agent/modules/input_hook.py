...
# ---------------------------------------------------------------------------
# Standalone foreground mode (sends keystrokes to service via HTTP)
# ---------------------------------------------------------------------------

def run_standalone(service_url: str = "http://127.0.0.1:8330") -> None:
    """Run input hook in the foreground, sending data to the service via HTTP.

    This is needed because CGEventTap requires window server access,
    which background daemon processes don't have.
    """
    try:
        import Quartz  # type: ignore[import-not-found]
    except ImportError:
    ...
    def _flush() -> None:
        nonlocal last_flush_time
        with buffer_lock:
            if len("".join(text_buffer)) < min_length:
                text_buffer.clear()
                return
        ...
        try:
            httpx.post(
                f"{service_url}/ingest/claudecode",
                json={"text": f"Typed text: {text}"},
                timeout=3,
            )
            _hook_status["flushes"] = int(_hook_status.get("flushes", 0)) + 1
            _hook_status["last_flush"] = datetime.now(tz=timezone.utc).isoformat()
    ...
    def _callback(proxy, event_type, event, refcon):  # noqa: ANN001, ARG001
        ...
        if chars:
            with buffer_lock:
                text_buffer.append(chars)
            _hook_status["keystrokes"] = int(_hook_status.get("keystrokes", 0)) + len(chars)
    ...
    if tap is None:
        print("Error: Failed to create CGEventTap. Check accessibility permissions.")
        _hook_status["active"] = False
        return
    ...
    print("Input hook running (foreground). Press Ctrl+C to stop.")
    try:
        while running.is_set():
            Quartz.CFRunLoopRunInMode(Quartz.kCFRunLoopDefaultMode, 1.0, False)
