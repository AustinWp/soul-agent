...
def _clipboard_loop(engine: VaultEngine, running: threading.Event, ingest_queue: IngestQueue | None = None) -> None:
    """Poll clipboard and ingest new content.

    When *ingest_queue* is provided, items are placed on the queue for
    classification. Otherwise, appends directly to the daily log.
    """
    ...
    while running.is_set():
        ...
        try:
            ...
            if ingest_queue is not None:
                ...
                engine.append_log(truncated, source="clipboard")
...
def start_clipboard_monitor(engine: VaultEngine, ingest_queue: IngestQueue | None = None) -> tuple[threading.Thread, threading.Event]:
